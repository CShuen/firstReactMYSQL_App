...
function App() {
  ...
  const getProdct = () => {
    axios
      .get("http://localhost:3001/api/get_product")
      .then((res) => {
  ...
  const deleteProduct = (id) => {
    axios
      .delete(`http://localhost:3001/api/delete_product/${id}`)
      .then((res) => {
  ...
  const updateProduct = (id) => {
    ...
    axios
      .put(`http://localhost:3001/api/update_product/${id}`, {
          ...
          product.map((prod) => {
            return (
              <div key={prod.id + prod.product_name} className="prod-row">
                ...
                <div style={{ marginBottom: "30px" }}>{prod.description}</div>
                <input
                  type={"text"}
                  placeholder="Updat Description"
                  defaultValue={""}
                  onClick={() => {
                    setOnchangeId(prod.id);
                  }}
